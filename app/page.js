...
import { FaArrowRight, FaGlobe, FaChevronDown, FaCrown } from "react-icons/fa";
...
const Home = () => {
  ...
  return (
    <main className="min-h-screen flex flex-col pb-32 relative bg-transparent text-[#001011] overflow-hidden">
      ...
      <section
        id="heroSection"
        className="relative w-[100%] mt-[40px] md:mt-[80px] overflow-hidden flex items-center h-[650px] md:h-[700px] shadow-[0_40px_100px_rgba(0,16,17,0.15)] group/hero backdrop-blur-md"
      >
        ...
        <div className="relative w-full z-10 px-8 md:px-16 lg:px-24 py-24 text-white flex flex-col justify-center h-full">
          ...
          {/* Main Headline */}
          <h1 className="text-4xl sm:text-6xl md:text-8xl lg:text-[8.5rem] font-black max-w-[1200px] leading-[0.95] tracking-tight drop-shadow-[0_10px_30px_rgba(0,0,0,0.5)] mb-10 text-white font-[var(--worksans-font)]">
            St. Mary <br />
          ...
          <div className="mt-14 flex flex-col sm:flex-row flex-wrap gap-6 relative z-20">
            <button onClick={()=>router.push("/apply")} className="bg-gradient-to-r from-[#cc5500] to-[#ff8c00] text-white px-10 md:px-16 py-4 md:py-6 rounded-full font-black flex items-center justify-center gap-4 transition-all shadow-[0_15px_40px_rgba(204,85,0,0.5)] hover:-translate-y-2 hover:shadow-[0_25px_60px_rgba(204,85,0,0.7)] text-xs md:text-[1.3rem] tracking-[0.2em] relative overflow-hidden group/btn group active:scale-95 w-full sm:w-max font-[var(--worksans-font)]">
            ...
            {/* <button className="bg-white/5 backdrop-blur-xl text-white border-2 border-white/20 px-10 md:px-16 py-4 md:py-6 rounded-full font-bold flex items-center justify-center gap-4 hover:bg-white hover:text-[#000a18] transition-all shadow-[0_10px_40px_rgba(0,0,0,0.2)] hover:-translate-y-2 hover:shadow-2xl text-xs md:text-sm uppercase tracking-[0.2em] w-full sm:w-max group/visit relative overflow-hidden font-[var(--worksans-font)] active:scale-95">
              <FaGlobe
                size={20}
                className="relative z-10 text-[#00BFFF] group-hover/visit:text-[#000a18] transition-colors"
              />
